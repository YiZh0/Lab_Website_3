...
app.get('/team_stats', function(req, res) {
	var fall_2018 =  'select * from football_games;';
...
app.get('/player_info', function(req, res) {
	var queryi = 'select id, name from football_players;';
	db.any(queryi)
...
app.get('/player_info/select_player', function(req, res) {
	var splayerid = req.query.player_choice;
	var query_1 = 'select id, name from football_players;';
	var query_2 = "select * from football_players where id = " + splayerid + ";";
	var query_3 = "SELECT COUNT(*) FROM football_games WHERE " + splayerid + " = ANY(players);";
	db.task('get-everything', task => {
        return task.batch([
            task.any(query_1),
            task.any(query_2),
